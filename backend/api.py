...
# Resolve base directory for data/outputs both in local dev (project/backend)
# and inside Docker (where code is copied/mounted to /app).
_current_dir = Path(__file__).resolve().parent
if (_current_dir / "data").exists() or (_current_dir / "outputs").exists():
    BASE_DIR = _current_dir
elif (_current_dir.parent / "data").exists() or (_current_dir.parent / "outputs").exists():
    BASE_DIR = _current_dir.parent
else:
    # Fallback to current dir; in Docker we mount ./data and ./outputs into /app
    BASE_DIR = _current_dir
